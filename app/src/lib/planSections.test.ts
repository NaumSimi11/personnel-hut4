...
describe('planSections', () => {
  it('lists every section of a fully visible onboarding plan, in page order', () => {
    expect(ids(full)).toEqual(['checklist', 'welcome', 'kit', 'handover'])
  })
  ...
  it('drops the welcome note and starter kit when someone is leaving', () => {
    expect(ids({ ...full, kind: 'offboarding' })).toEqual(['checklist', 'handover'])
  })
  ...
  it('hides the starter kit without the IT capability', () => {
    expect(ids({ ...full, canViewIt: false })).toEqual(['checklist', 'welcome', 'handover'])
  })
